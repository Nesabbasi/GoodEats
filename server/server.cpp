...
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h>  //incomplete in msvc (and a little in mingw)
#include <sys/types.h> //incomplete in msvc
...
#else
//POSIX sockets
//#include <dirent.h> //mingw. not in msvc
...
Server::Server(int _port) : port(_port) {
  ...
  #endif
  if (!ISVALIDSOCKET(sc))
    throw Exception("Error on opening socket. errno: " + to_string(GETSOCKETERRNO()));
  ...
  if (::bind(sc, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) != 0) {
    throw Exception("Error on binding. errno: " + to_string(GETSOCKETERRNO()));
  }
}
...
void Server::run() {
  ...
  while (true) {
    ...
    if (!ISVALIDSOCKET(newsc))
      throw Exception("Error on accept. errno: " + to_string(GETSOCKETERRNO()));
    Response* res = NULL;
    try {
    ...
    if (wr != si)
      throw Exception("Send error. errno: " + to_string(GETSOCKETERRNO()));
    CLOSESOCKET(newsc);
  }
}
