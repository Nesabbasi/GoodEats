...
TemplateParser::TemplateParser(string _filePath) {
  filePath = _filePath;
  variableCount = 0;
  programName = to_string(TemplateParser::lastParserNum) + system_fileExtention;
...
void TemplateParser::compileCode() {
  ...
  string cmd = system_makeDirectory + outputFolder + " && " + cc + " " + toCompileFile +
               " " + utilitiesPath + " -o " + outputFolder + system_slash + programName +
               " && " + system_removeFile + toCompileFile;
...
string TemplateParser::runGeneratedCode() {
  ...
  string cmd =
      system_terminalProgramStart + outputFolder + system_slash + programName + " " + " > " + staticTemplate;
  string error = "Error in running template  " + filePath;
  TemplateUtils::runSystemCommand(cmd, error);

  string html = readFile(staticTemplate);

  cmd = system_removeFile + staticTemplate;
...
void TemplateParser::deleteExecutable() {
  string cmd = system_removeFile + outputFolder + "/" + programName;
...
void TemplateParser::deleteLocalTemplate() {
  string cmd = system_removeFile + outputFolder + "/" + localTemplate(parserNum);
...
void TemplateParser::TemplateUtils::runSystemCommand(string command,
  ...
  #ifdef _WIN32
  if (ret != 0 && command.substr(0, 5) != "mkdir") {
